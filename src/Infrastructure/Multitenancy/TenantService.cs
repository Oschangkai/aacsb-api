...
using AACSB.WebApi.Infrastructure.Persistence;
...
{
    private readonly IMultiTenantStore<AACSBTenantInfo> _tenantStore;
    ...
    public TenantService(
        IMultiTenantStore<AACSBTenantInfo> tenantStore,
    ...
    public async Task<string> CreateAsync(CreateTenantRequest request, CancellationToken cancellationToken)
    {
        ...
        var tenant = new AACSBTenantInfo(request.Id, request.Name, request.ConnectionString, request.AdminEmail, request.Issuer);
    ...
    private async Task<AACSBTenantInfo> GetTenantInfoAsync(string id) =>
        await _tenantStore.TryGetAsync(id)
            ?? throw new NotFoundException(_t["{0} {1} Not Found.", typeof(AACSBTenantInfo).Name, id]);
}