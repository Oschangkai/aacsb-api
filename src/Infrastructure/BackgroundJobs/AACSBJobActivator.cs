...
using AACSB.WebApi.Infrastructure.Common;
...
namespace AACSB.WebApi.Infrastructure.BackgroundJobs;

public class AACSBJobActivator : JobActivator
{
    private readonly IServiceScopeFactory _scopeFactory;

    public AACSBJobActivator(IServiceScopeFactory scopeFactory) =>
    ...
    private class Scope : JobActivatorScope, IServiceProvider
    {
        ...
        {
            var tenantInfo = _context.GetJobParameter<AACSBTenantInfo>(MultitenancyConstants.TenantIdName);
            if (tenantInfo is not null)
            {
                _scope.ServiceProvider.GetRequiredService<IMultiTenantContextAccessor>()
                    .MultiTenantContext = new MultiTenantContext<AACSBTenantInfo>
                    {
